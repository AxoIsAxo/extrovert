...
### A1. Avatar double-`/uploads/` prefix in API serialization  [P1]
...
### A2. `avatar: null` hardcoded in notifications and status context  [P1]
...
### A3. Notifications cursor accepted but ignored  [P1]
...
### A4. Idempotency keys never expire (unbounded growth)  [P1]
...
### A5. `media_attachments` dimensions never populated  [P2]
...
### A6. Refresh token TTL == access token TTL (24h)  [P2]
...
### A7. Rate-limit spec is wrong (per-IP, not per-token)  [P1]
...
### A8. `trust proxy 1` + no X-Forwarded-For validation = IP spoofing  [P1]
...
### A9. `picture` claim in id_token/userinfo is a relative path  [P1]
...
### B1. Scope requirements for `/accounts/*_credentials`  [P2]
...
### B2. `token_endpoint_auth_methods_supported` omits `none`  [P2]
...
### B3. Undocumented endpoints missing from `api-spec.js`  [P2]
...
### B4. `response_modes_supported` advertises `fragment` but code doesn't support it  [P3]
...
### B5. Mixed timestamp units undocumented  [P2]
...
### B6. `statuses_count` missing from Account  [P2]
...
### B7. `claims_supported` in discovery doc  [P2]
...
### C1. DM REST API (E2E-encrypted) — largest item  [User decision]
...
### C3. Profile update extend fields  [P2]
...
### C4. Account relationships endpoint  [P2]
...
### C5. Realtime notifications (SSE)  [User decision]
...
### C6. Unread notification count endpoint  [P2]
...
### D1. Media served publicly with no access control  [User decision]
...
### D2. OIDC private key stored in plaintext on disk  [P1]
...
### D3. No JWT key rotation mechanism  [P2]
...
### D4. Refresh-token reuse detection doesn't revoke the family  [P3]
...
### D5. Validate session cookie auth for web forms  [P2]
...
### D6. Helmet / security headers audit  [P2]
...
### E1. `db.js` is 1200+ lines of raw SQL — modularize  [P3]
...
### E2. No automated tests covering the API surface  [P1]
...
### E3. `feed.buildFeed` runs O(N) per request with no caching  [P2]
...
### E4. `serializePost` runs N+1 `COUNT(*)` queries per post  [P2]
...
### E5. `created_at: 0` synthesized accounts break sorting  [P2]
...
### E7. OpenAPI spec is hand-maintained and has drifted  [P2]
